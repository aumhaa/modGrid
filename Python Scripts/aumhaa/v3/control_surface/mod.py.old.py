...
# class ParamHolder(Component):
	...
	def _value_change(self, *a):
		# self.notify_display_value(self.display_value)
		self.notify_display_value(self.display_value, self)
		if self._send_via_pipe:
			self._send_pipe_notification()
	
	def _send_pipe_notification(self):
...
class ModDeviceProxy(ControlManager, EventObject):
	...
	def test_dict(self):
		for bank_type in list(self._bank_dict.keys()):
...
class ModGridDeviceProxy(ModDeviceProxy):
	...
	device_name_data_source = DisplayDataSource('')
	bank_name_data_source = DisplayDataSource('')
...
class ModClient(NotifyingControlElement):
	...
	def _on_parameters_changed(self):
		self.notify_parameter_data()
		self._on_parameters_value_changed.replace_subjects(self._device_proxy._params)
		self._on_parameters_name_changed.replace_subjects(self._device_proxy._params)


	@listens_group('display_name')
	def _on_parameters_name_changed(self, name, param):
		# debug('_on_parameters_name_changed', *a)
		self.notify_parameter_data({'name': {param._index, name}})


	@listens_group('display_value')
	def _on_parameters_value_changed(self, value, param, *a):
		# str_for_value = param._parameter.str_for_value(value)
		# debug('_on_parameters_value_changed', param._parameter.name, str_for_value)
		# self.notify_parameter_data({'value': {param._index: str_for_value}})
		self.notify_parameter_data({'value': {param._index: value}})
	...
	def parameter_data(self):
		...
		# displays = {index: param.display_value for param, index in enumerate(device._params)}
		# displays = {value:{index : param.display_value for index, param in enumerate(params)}}
	    # displays = {name:{index : param.display_name for index, param in enumerate(params)}}
		names = {index : param.display_name for index, param in enumerate(params)}
		values = {index : param.display_value for index, param in enumerate(params)}
		displays = {'name':names, 'value':values}
		# displays = [param.display_value for param in device._params] #+ (param.value_to_string for param in device._params)
